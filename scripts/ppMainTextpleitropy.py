...
def mutate(off, u_adapt, alpha, n, mut, pleit):
	"""
	...
	if pleit == 1:
	#with pleitropy: 
		zero = np.array([0, 1]) #make a an array with 0 and 1, randomly chosen
		zero_repeat = np.tile(zero, len(newmuts)).reshape(len(newmuts), n)
		zero_shuffled = shuffle_along_axis(zero_repeat, 1) #randomly shuffle the array 
		newmuts = zero_shuffled * newmuts 
...
opt_dists = [0.5, 1, 1.5] #distance to optima
...
def main():
	...
	[fileHandle_A, fileHandle_B, fileHandle_C] = open_output_files(ns, N_adapts, alpha_adapts, sigma_adapts, opt_dists, dom)


	#loop over pleitropy
	i_ple = 0 
	while i_ple < len(pleitropy):
		pleit = pleitropy[i_ple]
					...
					while i_alpha < len(alpha_adapts):
						...
						while i_opt_dist < len(opt_dists):
							...
							while l < len(ns):
								...
								theta2_list = np.array([[opt_dist, 0] for x in angles]) #optima to use
								...
								while j < len(theta2_list):
									...
									while rep < nreps:
										...
										while gen < maxgen + 1:
											...
											[pop1_newmuts, pop1_genotype, pop1_overall, mut1] = mutate(off1, u_adapt, alpha_adapt, n, mut1, pleit)
											[pop2_newmuts, pop2_genotype, pop2_overall, mut2] = mutate(off2, u_adapt, alpha_adapt, n, mut2, pleit)
										...
										pheno_data = np.column_stack([np.array([i+1 for i in range(len(hybrid_pheno))]), np.array([rep+1]*len(hybrid_pheno)), np.array([round(angles[j]*180/math.pi,2)]*len(hybrid_pheno)), np.array([opt_dist]*len(hybrid_pheno)), np.reshape(np.repeat(h, np.shape(hybrid_pheno)[0], axis = 0), np.shape(hybrid_pheno)[0], 1), 
											np.reshape(np.repeat(alpha_adapt, np.shape(hybrid_pheno)[0], axis = 0), np.shape(hybrid_pheno)[0], 1), np.reshape(np.repeat(pleit, np.shape(hybrid_pheno)[0], axis = 0), np.shape(hybrid_pheno)[0], 1), phenos1, phenos2, hybrid_pheno, F2_phenotype])
										...
										adapt_summary = np.vstack((adapt_summary_half))
										np.savetxt(fileHandle_C, adapt_summary, fmt = '%.3f', delimiter = ',')

										# print an update
										print('N = %d, sigma = %.3f, u = %.3f, alpha = %.3f, opt_dist = %.2f, n=%d, angle=%r, rep=%d, h=%d, pleitropy=%d ' %(N_adapt, sigma_adapt, u_adapt, alpha_adapt, opt_dist, n, round(angles[j]*180/math.pi,2), rep+1, h, pleit)) 
		...
		#go to the next pleitropy value
		i_ple += 1
	...
	df_pheno.columns = ['individuals', 'reps', 'angle', 'opt_dist', 'h', 'alpha', 'pleitropy', 'phenos1_1', 'phenos1_2', 'phenos2_1', 'phenos2_2', 'F1_hybrid_phenos1', 'F1_hybrid_phenos2', 'F2_hybrid_phenos1', 'F2_hybrid_phenos2']
